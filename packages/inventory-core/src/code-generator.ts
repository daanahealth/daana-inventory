...
export function renderCodeTemplate(
  ...
  return template.replace(/\{([^{}]+)\}/g, (_, raw: string) => {
    ...
    if (token === "LOCATION") return ctx.locationCode;
    if (token === "TYPE") return ctx.itemTypeName;
    if (token === "counter") return String(ctx.counter);
    ...
    if (token.startsWith("attr.")) {
      ...
      if (v === undefined || v === null) {
        throw new CodeTemplateError(
          `Template placeholder {${token}} references missing attribute "${key}"`
        );
      }
      return String(v);
    }
 ...
 */
export function createTemplateCodeGenerator(format: string): CodeGenerator {
  const probe: CodeGenerationContext = {
    itemTypeId: "probe",
    itemTypeName: "PROBE",
    locationCode: "PROBE",
    counter: 1,
...
export class InMemoryCodeGeneratorRegistry implements CodeGeneratorRegistry {
  private readonly byType = new Map<string, CodeGenerator>();
