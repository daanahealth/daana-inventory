...
// These are the canonical TS contracts that mirror the SQL schema in
// DaanaRx-Backend/migrations/002_core_inventory_platform.sql.
//
// Domain packs (e.g. @daana-health/domain-mass) extend these via the
// `attributes` JSONB blob, validated by per-item-type AttributeSchema.

// ---------------------------------------------------------------------------
// Status state-machine alphabet
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Attribute schema (JSON Schema typed)
// ---------------------------------------------------------------------------
...
export type AttributePropertySchema =
  | { type: "string"; enum?: readonly string[]; format?: string; description?: string }
  | { type: "number" | "integer"; minimum?: number; maximum?: number; description?: string }
  | { type: "boolean"; description?: string }
  | { type: "array"; items: AttributePropertySchema; description?: string }
  | { type: "object"; properties: Readonly<Record<string, AttributePropertySchema>>; required?: readonly string[]; description?: string }
  | { type: "null"; description?: string };

// ---------------------------------------------------------------------------
// Core entities (mirror SQL tables)
// ---------------------------------------------------------------------------
...
export interface ItemType {
  ...
  readonly createdAt: string; // ISO 8601 timestamptz
}
...
export interface Item {
  ...
  readonly expiryDate: string | null; // ISO 8601 date (YYYY-MM-DD)
...
// ---------------------------------------------------------------------------
// Item-type configuration (the bundle a domain pack registers)
// ---------------------------------------------------------------------------
