 ...
 */
export function compareFEFO(a: Item, b: Item): number {
  // 1. expiry_date ascending, nulls last
  const aExp = a.expiryDate;
  const bExp = b.expiryDate;
  if (aExp !== bExp) {
    if (aExp === null) return 1; // a has no expiry -> a after b
    if (bExp === null) return -1; // b has no expiry -> a before b
 ...
 */
export function sortFEFO(items: readonly Item[]): Item[] {
